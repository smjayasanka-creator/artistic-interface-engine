...
export const listEodRuns = createServerFn({ method: "GET" })
  ...
  .inputValidator((i: { branch_id?: string; limit?: number }) =>
    z.object({ branch_id: z.string().uuid().optional(), limit: z.number().int().min(1).max(200).optional() }).parse(i),
  )
  .handler(async ({ data, context }) => {
    let q = context.supabase
      .from("eod_run")
      .select("id, branch_id, business_date, status, initiated_by, initiated_at, approved_by, approved_at, started_at, completed_at, duration_ms, steps, warnings, pre_check, reports")
...
export const runPreCheck = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: r, error } = await context.supabase.rpc("eod_precheck" as any, {
...
export const initiateEod = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: id, error } = await context.supabase.rpc("eod_initiate" as any, {
      _branch_id: data.branch_id,
      _business_date: data.business_date,
    } as any);
    if (error) throw new Error(error.message);
...
export const approveEod = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.rpc("eod_approve_and_run" as any, { _run_id: data.run_id } as any);
    if (error) throw new Error(error.message);
    return true;
  });
...
export const runStep = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: canProcess } = await context.supabase.rpc("has_permission" as any, {
      _user_id: context.userId,
      _permission: "eod.process",
    } as any);
    const { data: companyAdmin } = await context.supabase.rpc("is_company_admin" as any, {
    ...
    if (!error && data.step === "rollover") {
      await context.supabase.rpc("eod_finalize" as any, { _run_id: run.id, _status: "completed" } as any);
    }
...
async function stepLoanAccrual(ctx: Ctx) {
  ...
  const { data: loans } = await supabaseAdmin
    .from("loan")
    .select("id, principal, annual_rate_pct, disbursed_at, product_id, branch_id, product:product_id(accrued_interest_account_id, interest_income_account_id)")
    .eq("branch_id", branch_id)
    .eq("status", "active");
  let accrued = 0, skipped = 0, total = 0;
  for (const l of (loans ?? []) as any[]) {
    ...
    if (!accAcc || !incAcc || !disb || business_date <= disb) { skipped++; continue; }
    const { data: insts } = await supabaseAdmin.from("loan_installment").select("principal_paid").eq("loan_id", l.id);
    ...
    if (daily <= 0) { skipped++; continue; }

    const { data: existing } = await supabaseAdmin
      .from("loan_accrual").select("id").eq("loan_id", l.id).eq("accrual_date", business_date).maybeSingle();
    if (existing) { skipped++; continue; }

    const { data: prev } = await supabaseAdmin
      .from("loan_accrual").select("cumulative_amount").eq("loan_id", l.id)
      .order("accrual_date", { ascending: false }).limit(1).maybeSingle();
    ...
    const { data: entryId, error: pe } = await supabaseAdmin.rpc("post_entry_system", {
      _company_id: company_id, _entry_date: business_date,
      _reference: `EOD-ACR-${business_date}`, _description: "EOD loan accrual",
      _lines: [{ account_id: accAcc, debit: daily, credit: 0 }, { account_id: incAcc, debit: 0, credit: daily }] as any,
      _branch_id: branch_id, _source_module: "loans", _source_ref: l.id,
      _idempotency_key: `eod-loan-accr:${l.id}:${business_date}`, _loan_id: l.id,
    });
    if (pe) { skipped++; continue; }
    await supabaseAdmin.from("loan_accrual").insert({
      loan_id: l.id, company_id, accrual_date: business_date,
      outstanding_principal: outstanding, daily_amount: daily,
      cumulative_amount: cumulative, entry_id: entryId,
    });
    accrued++; total += daily;
  }
  return { loans_scanned: loans?.length ?? 0, accrued, skipped, total_amount: Number(total.toFixed(2)) };
}
...
async function stepFdAccrual(ctx: Ctx) {
  ...
  const { data: deposits } = await supabaseAdmin
    ...
    .eq("branch_id", branch_id).eq("status", "active");
  let accrued = 0, skipped = 0, total = 0;
  for (const d of (deposits ?? []) as any[]) {
    if (business_date < d.value_date || business_date > d.maturity_date) { skipped++; continue; }
    const daily = Number(((Number(d.principal) * Number(d.rate_at_booking)) / 365).toFixed(2));
    const { data: prev } = await supabaseAdmin.from("fd_accrual")
      .select("cumulative_amount").eq("deposit_id", d.id)
      .order("accrual_date", { ascending: false }).limit(1).maybeSingle();
    const cumulative = Number(((Number(prev?.cumulative_amount ?? 0)) + daily).toFixed(2));
    const { error: ie } = await supabaseAdmin.from("fd_accrual").insert({
      deposit_id: d.id, accrual_date: business_date, daily_amount: daily, cumulative_amount: cumulative,
    });
    if (ie) { skipped++; continue; }
    accrued++; total += daily;
  }
  return { deposits_scanned: deposits?.length ?? 0, accrued, skipped, total_amount: Number(total.toFixed(2)) };
}
...
async function stepPenaltyCharges(ctx: Ctx) {
  ...
  return { overdue_installments: overdueCount, note: "Penalty posting requires per-product penalty rules; counted only." };
}

async function stepParNpa(ctx: Ctx) {
  const { supabaseAdmin, branch_id, business_date } = ctx;
  const { data: loans } = await supabaseAdmin
    .from("loan").select("id").eq("branch_id", branch_id).eq("status", "active");
  const buckets = { par1: 0, par30: 0, par60: 0, par90: 0, par180: 0 };
  let scanned = 0;
  for (const l of (loans ?? []) as any[]) {
    scanned++;
    const { data: earliest } = await supabaseAdmin
      .from("loan_installment").select("due_date, principal_due, principal_paid")
      .eq("loan_id", l.id).lt("due_date", business_date)
    ...
    const days = Math.floor((new Date(business_date).getTime() - new Date(earliest.due_date).getTime()) / 86400000);
...
async function stepFdMaturity(ctx: Ctx) {
  ...
  const { data: matured } = await supabaseAdmin
    ...
    .eq("branch_id", branch_id).eq("status", "active").eq("maturity_date", business_date);
  return {
    matured_today: matured?.length ?? 0,
    auto_renewal: (matured ?? []).filter((d: any) => d.maturity_instruction === "auto_renewal").length,
...
async function stepSavingsInterest(ctx: Ctx) {
  const { supabaseAdmin, branch_id, business_date } = ctx;
  const isMonthEnd = business_date === new Date(new Date(business_date).getFullYear(), new Date(business_date).getMonth() + 1, 0).toISOString().slice(0, 10);
  if (!isMonthEnd) return { skipped: true, reason: "not_month_end" };
  const { data: accts } = await supabaseAdmin
    .from("savings_account").select("id, balance").eq("branch_id", branch_id).eq("status", "active");
  return { accounts_scanned: accts?.length ?? 0, note: "Interest posting requires product-level rate config; scanned only." };
}

async function stepGlPost(ctx: Ctx) {
  const { supabaseAdmin, branch_id, business_date } = ctx;
  const { count } = await supabaseAdmin
    .from("journal_entry").select("*", { count: "exact", head: true })
    .eq("branch_id", branch_id).eq("entry_date", business_date);
  return { entries_today: count ?? 0 };
}

async function stepTrialBalance(ctx: Ctx) {
  const { supabaseAdmin, company_id, business_date } = ctx;
  const { data: rows } = await supabaseAdmin.rpc("compute_trial_balance" as any, {
    _company_id: company_id, _as_at: business_date,
  } as any).then((r: any) => r).catch(() => ({ data: null }));
  if (!rows) {
    // Fallback: aggregate postings directly.
    const { data: agg } = await supabaseAdmin
      .from("posting")
      .select("account_id, debit, credit, entry:entry_id!inner(entry_date, branch:branch_id!inner(company_id))")
      .lte("entry.entry_date", business_date)
      .eq("entry.branch.company_id", company_id);
    let dr = 0, cr = 0;
    for (const p of (agg ?? []) as any[]) { dr += Number(p.debit ?? 0); cr += Number(p.credit ?? 0); }
    const balanced = Math.abs(dr - cr) < 0.01;
    if (!balanced) throw new Error(`Trial balance out of balance: DR ${dr.toFixed(2)} vs CR ${cr.toFixed(2)}`);
...
async function stepReports(ctx: Ctx) {
  ...
  const [{ count: txnCount }, { count: loanCount }, { count: fdCount }] = await Promise.all([
    supabaseAdmin.from("journal_entry").select("*", { count: "exact", head: true }).eq("branch_id", branch_id).eq("entry_date", business_date),
    supabaseAdmin.from("loan").select("*", { count: "exact", head: true }).eq("branch_id", branch_id).eq("status", "active"),
    supabaseAdmin.from("fixed_deposit").select("*", { count: "exact", head: true }).eq("branch_id", branch_id).eq("status", "active"),
  ]);
  const reports = {
...
export const runAllSteps = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const step of STEPS) {
      const { data: run } = await context.supabase.from("eod_run").select("steps, status").eq("id", data.run_id).maybeSingle();
      if (!run) throw new Error("Run not found");
      ...
      results.push({ step, ...r });
      if (!r.ok) {
        await context.supabase.rpc("eod_finalize" as any, { _run_id: data.run_id, _status: "failed" } as any);
        break;
      }
    }
    return results;
...
async function runStepInternal(context: any, run_id: string, step: StepKey) {
  const { data: run } = await context.supabase
    .from("eod_run").select("company_id, branch_id, business_date").eq("id", run_id).maybeSingle();
  ...
  try {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const ctx: Ctx = { supabaseAdmin, run_id, company_id: run.company_id, branch_id: run.branch_id, business_date: run.business_date };
    switch (step) {
      case "loan_accrual":     metrics = await stepLoanAccrual(ctx); break;
      case "fd_accrual":       metrics = await stepFdAccrual(ctx); break;
      case "penalty_charges":  metrics = await stepPenaltyCharges(ctx); break;
  ...
  await context.supabase.rpc("eod_record_step" as any, {
    _run_id: run_id, _step_key: step,
    _status: error ? "failed" : "completed",
    _metrics: metrics, _error: error,
  } as any);
  if (!error && step === "rollover") {
    await context.supabase.rpc("eod_finalize" as any, { _run_id: run_id, _status: "completed" } as any);
  }
