  ...
  .handler(async ({ context }) => {
    ...
    const [
      ...
      supabase
        ...
        .select("company_id, status, mrr, currency, plan_id, plan:plan_id(code,name)"),
      ...
      supabase
        .from("loan")
        .select("id, principal, status, branch_id, branch:branch_id(company_id)"),
      supabase
        .from("fixed_deposit")
        .select("id, principal, status, branch_id, branch:branch_id(company_id)"),
    ]);
    ...
    for (const s of staff ?? []) {
      const c = (s as any).branch?.company_id;
      if (c) b(c).staff++;
    }
    for (const c of clients ?? []) {
      const co = (c as any).branch?.company_id;
      if (co && c.status === "active") b(co).clients++;
    }
    for (const l of loans ?? []) {
      const co = (l as any).branch?.company_id;
      if (co) {
        b(co).loanCount++;
        b(co).loanValue += Number(l.principal || 0);
      }
    }
    for (const f of fds ?? []) {
      const co = (f as any).branch?.company_id;
      if (co) {
        b(co).fdCount++;
        b(co).fdValue += Number(f.principal || 0);
      }
    }
...
export const upsertCompanySubscription = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error } = await context.supabase.from("company_subscription").upsert(
      {
