...
export const API_CONTRACTS: ApiContract[] = [
  ...
  {
    id: "loan_applications.documents.upload",
    method: "POST",
    path: "/api/public/v1/loan-applications/{id}/documents",
    resource: "loan_applications",
    title: "Upload document to loan application",
    summary:
      "Upload a supporting document (identity, deed, invoice, evaluation report, etc.) to a loan application. Send the file as base64 in content_base64 — max ~8 MB decoded. Stored in the private loan-application-documents bucket; only reachable via signed download URLs returned by the Get document endpoint.",
    scope: "loan_applications.write",
    direction: "inbound",
    status: "live",
    requiresIdempotency: false,
    request: LoanApplicationDocumentUploadRequest,
    response: LoanApplicationDocumentUploadResponse,
    requestExample: {
    ...
    responseExample: {
      ...
      size_bytes: 184320,
      version: 1,
      uploaded_at: "2026-07-22T10:15:00.000Z",
    },
    fields: [
      { path: "id", label: "Application id", type: "uuid", required: true, inbound: true, notes: "Path parameter." },
      { path: "document_type", label: "Document type slug", type: "string", required: true, inbound: true },
      { path: "file_name", label: "Original file name", type: "string", required: true, inbound: true },
      { path: "mime_type", label: "MIME type", type: "string", inbound: true },
      { path: "content_base64", label: "File bytes (base64)", type: "string", required: true, inbound: true, notes: "No data: URI prefix. Max ~11.5 MB base64 (~8 MB binary)." },
      { path: "id", label: "Document id", type: "uuid", outbound: true },
      { path: "application_id", label: "Application id", type: "uuid", outbound: true },
      { path: "application_no", label: "Application no.", type: "string", outbound: true },
      { path: "size_bytes", label: "Decoded size", type: "int", outbound: true },
      { path: "version", label: "Version", type: "int", outbound: true, notes: "Auto-incremented per (application, document_type)." },
    ],
    errors: [
      ...COMMON_ERRORS,
      { code: 404, error: "not_found", meaning: "Loan application not found in caller's company." },
      { code: 413, error: "payload_too_large", meaning: "content_base64 exceeds the size limit." },
      { code: 422, error: "invalid_base64", meaning: "content_base64 is not valid base64." },
    ],
  },
  {
    id: "loan_applications.documents.list",
    method: "GET",
    path: "/api/public/v1/loan-applications/{id}/documents",
    resource: "loan_applications",
    title: "List documents on a loan application",
    summary: "List all documents attached to a loan application, ordered by uploaded_at DESC.",
    scope: "loan_applications.read",
    direction: "outbound",
    status: "live",
    requiresIdempotency: false,
    response: LoanApplicationDocumentListResponse,
    responseExample: {
      data: [
        {
          id: "b1e2…",
    ...
    fields: [
      { path: "id", label: "Application id", type: "uuid", required: true, inbound: true, notes: "Path parameter." },
      { path: "data", label: "Documents", type: "array", outbound: true },
    ],
    errors: [
      ...COMMON_ERRORS,
      { code: 404, error: "not_found", meaning: "Loan application not found in caller's company." },
  ...
  {
    id: "loan_applications.documents.get",
    method: "GET",
    path: "/api/public/v1/loan-applications/{id}/documents/{documentId}",
    resource: "loan_applications",
    title: "Get loan application document (with signed download URL)",
    summary:
      "Fetch a single document's metadata plus a short-lived signed download URL (300 s). The URL requires no auth and works from any client; regenerate by calling this endpoint again.",
    scope: "loan_applications.read",
    direction: "outbound",
    status: "live",
    requiresIdempotency: false,
    response: LoanApplicationDocumentDetailResponse,
    responseExample: {
      id: "b1e2…",
      application_id: "9f0e…",
      application_no: "AP000123",
      ...
      download_url_expires_at: "2026-07-22T10:20:00.000Z",
    },
    fields: [
      { path: "id", label: "Application id", type: "uuid", required: true, inbound: true, notes: "Path parameter." },
      { path: "documentId", label: "Document id", type: "uuid", required: true, inbound: true, notes: "Path parameter." },
      { path: "download_url", label: "Signed download URL", type: "string", outbound: true },
      { path: "download_url_expires_at", label: "URL expiry", type: "datetime", outbound: true },
    ],
    errors: [
      ...COMMON_ERRORS,
      { code: 404, error: "not_found", meaning: "Document (or its parent application) not found in caller's company." },
    ],
  },
  {
    id: "loan_applications.documents.delete",
    method: "DELETE",
    path: "/api/public/v1/loan-applications/{id}/documents/{documentId}",
    resource: "loan_applications",
    title: "Delete loan application document",
    summary:
      "Remove a document from a loan application. Deletes both the storage object and the metadata row. Idempotent — deleting an already-removed id returns 404.",
    scope: "loan_applications.write",
    direction: "inbound",
    status: "live",
    requiresIdempotency: false,
    response: LoanApplicationDocumentDeleteResponse,
    responseExample: { status: "deleted", id: "b1e2…", application_id: "9f0e…" },
    fields: [
      { path: "id", label: "Application id", type: "uuid", required: true, inbound: true, notes: "Path parameter." },
      { path: "documentId", label: "Document id", type: "uuid", required: true, inbound: true, notes: "Path parameter." },
    ],
  ...
  {
    id: "clients.attachments.upload",
    method: "POST",
    path: "/api/public/v1/clients/{id}/attachments",
    resource: "clients",
    title: "Upload attachment to client",
    summary:
      "Upload a client-level document (identity, KYC, photo, address proof, etc.) as base64. Max ~8 MB decoded. Stored in the private client-attachments bucket; download via the Get attachment endpoint.",
    scope: "clients.write",
    direction: "inbound",
    status: "live",
    requiresIdempotency: false,
    request: ClientAttachmentUploadRequest,
    response: ClientAttachmentUploadResponse,
    requestExample: {
      document_type: "national_id_front",
      file_name: "nic-front.jpg",
      mime_type: "image/jpeg",
    ...
    fields: [
      ...
      { path: "mime_type", label: "MIME type", type: "string", inbound: true },
      { path: "content_base64", label: "File bytes (base64)", type: "string", required: true, inbound: true, notes: "No data: URI prefix. Max ~11.5 MB base64 (~8 MB binary)." },
      { path: "id", label: "Attachment id", type: "uuid", outbound: true },
      { path: "client_id", label: "Client id", type: "uuid", outbound: true },
      { path: "version", label: "Version", type: "int", outbound: true, notes: "Auto-incremented per (client, document_type)." },
    ],
    errors: [
      ...COMMON_ERRORS,
      { code: 404, error: "not_found", meaning: "Client not found in caller's company." },
  ...
  {
    id: "clients.attachments.list",
    method: "GET",
    path: "/api/public/v1/clients/{id}/attachments",
    resource: "clients",
    title: "List client attachments",
    summary: "List all attachments on a client, ordered by uploaded_at DESC.",
    scope: "clients.read",
    direction: "outbound",
    status: "live",
    requiresIdempotency: false,
    response: ClientAttachmentListResponse,
    responseExample: { data: [] },
    fields: [
      { path: "id", label: "Client id", type: "uuid", required: true, inbound: true, notes: "Path parameter." },
      { path: "data", label: "Attachments", type: "array", outbound: true },
    ],
    errors: [
      ...COMMON_ERRORS,
      { code: 404, error: "not_found", meaning: "Client not found in caller's company." },
  ...
  {
    id: "clients.attachments.get",
    method: "GET",
    path: "/api/public/v1/clients/{id}/attachments/{attachmentId}",
    resource: "clients",
    title: "Get client attachment (with signed download URL)",
    summary:
      "Fetch a single attachment's metadata plus a short-lived signed download URL (300 s). Regenerate by calling this endpoint again.",
    scope: "clients.read",
    direction: "outbound",
    status: "live",
    requiresIdempotency: false,
    response: ClientAttachmentDetailResponse,
    responseExample: {
      id: "a1b2…",
      client_id: "9f0e…",
      document_type: "national_id_front",
    ...
    fields: [
      { path: "id", label: "Client id", type: "uuid", required: true, inbound: true, notes: "Path parameter." },
      { path: "attachmentId", label: "Attachment id", type: "uuid", required: true, inbound: true, notes: "Path parameter." },
      { path: "download_url", label: "Signed download URL", type: "string", outbound: true },
      { path: "download_url_expires_at", label: "URL expiry", type: "datetime", outbound: true },
    ],
    errors: [
  ...
  {
    id: "clients.attachments.delete",
    method: "DELETE",
    path: "/api/public/v1/clients/{id}/attachments/{attachmentId}",
    resource: "clients",
    title: "Delete client attachment",
    summary:
      "Remove an attachment from a client. Deletes both the storage object and the metadata row. Idempotent — deleting an already-removed id returns 404.",
    scope: "clients.write",
    direction: "inbound",
    status: "live",
    requiresIdempotency: false,
    response: ClientAttachmentDeleteResponse,
    responseExample: { status: "deleted", id: "a1b2…", client_id: "9f0e…" },
    fields: [
      { path: "id", label: "Client id", type: "uuid", required: true, inbound: true, notes: "Path parameter." },
      { path: "attachmentId", label: "Attachment id", type: "uuid", required: true, inbound: true, notes: "Path parameter." },
    ],
