...
export const createLoanApplication = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: br, error: brErr } = await supabase
      .from("branch").select("company_id").eq("id", data.branch_id).maybeSingle();
...
export const listLoanApplications = createServerFn({ method: "GET" })
  ...
  .inputValidator((i: { status?: string; limit?: number } = {}) =>
    z.object({ status: z.string().optional(), limit: z.number().int().max(500).optional() }).parse(i))
  .handler(async ({ data, context }) => {
    const { supabase } = context;
    let q = supabase.from("loan_application" as any)
      .select("id, application_no, status, requested_principal, requested_tenor_months, currency, purpose, submitted_at, decided_at, disbursed_at, loan_id, client_id, product_id, branch_id, created_at, client:client(id, full_name), product:loan_product(id, name), branch:branch(id, code, name)")
...
export const getLoanApplication = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase } = context;
    const [master, applicants, evaluation, employment, business, existing, guarantors, collateral, documents, approvals, notes, history] = await Promise.all([
      supabase.from("loan_application" as any).select("*, client:client(id, full_name), product:loan_product(id, name), branch:branch(id, code, name)").eq("id", data.id).maybeSingle(),
      supabase.from("loan_application_applicant" as any).select("*").eq("application_id", data.id),
      supabase.from("loan_application_evaluation" as any).select("*").eq("application_id", data.id).maybeSingle(),
      supabase.from("loan_application_employment" as any).select("*").eq("application_id", data.id),
      supabase.from("loan_application_business" as any).select("*").eq("application_id", data.id),
      supabase.from("loan_application_existing_facility" as any).select("*").eq("application_id", data.id),
      supabase.from("loan_application_guarantor" as any).select("*").eq("application_id", data.id),
      supabase.from("loan_application_collateral" as any).select("*, security_type:security_type(id, name)").eq("application_id", data.id),
      supabase.from("loan_application_document" as any).select("*").eq("application_id", data.id).order("uploaded_at", { ascending: false }),
      supabase.from("loan_application_approval" as any).select("*").eq("application_id", data.id).order("decided_at", { ascending: true }),
      supabase.from("loan_application_note" as any).select("*").eq("application_id", data.id).order("created_at", { ascending: false }),
      supabase.from("loan_application_status_history" as any).select("*").eq("application_id", data.id).order("created_at", { ascending: true }),
    ]);
...
export const addApplicationDocument = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: {
    application_id: string; document_type: string; file_name: string;
    storage_path: string; storage_bucket?: string; mime_type?: string; size_bytes?: number; version?: number;
  }) => z.object({
    application_id: z.string().uuid(),
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: app } = await supabase.from("loan_application" as any)
      .select("application_no").eq("id", data.application_id).maybeSingle();
    if (!app) throw new Error("Application not found");
    const { data: row, error } = await supabase.from("loan_application_document" as any).insert({
...
export const deleteApplicationDocument = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase } = context;
    const { error } = await supabase.from("loan_application_document" as any).delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const addApplicationNote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: app } = await supabase.from("loan_application" as any)
      .select("application_no").eq("id", data.application_id).maybeSingle();
    if (!app) throw new Error("Application not found");
    const { data: row, error } = await supabase.from("loan_application_note" as any).insert({
      application_id: data.application_id,
      application_no: (app as any).application_no,
      author_id: userId,
      note: data.note,
...
export const disburseApplication = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase } = context;
    const { data: loanId, error } = await supabase.rpc("disburse_loan_from_application" as any, {
      _application_id: data.application_id,
