...
export function TimeTravelTab() {
  ...
  function apply(iso: string | null) {
    ...
    toast.success(
      iso ? `Time travelled to ${new Date(iso).toLocaleString()}` : "Reset to real time",
    );
  }
  ...
  const diffDays = override ? Math.round((effective.getTime() - realNow.getTime()) / 86400000) : 0;
    ...
    <div className="flex flex-col gap-4">
      <Card>
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[12px] text-muted-foreground -mt-1">
              Overrides "today" for FD maturity, accruals, teller summary, and workflow SLA. Audit
              timestamps (approved_at, closed_at, created_at) always use real time.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <div
            className={`rounded-lg border p-3 ${override ? "border-amber-500/40 bg-amber-500/5" : "border-border"}`}
          >
            <div className="text-[11px] uppercase tracking-wide flex items-center gap-2">
              <span className={override ? "text-amber-700" : "text-muted-foreground"}>
                Effective time
              </span>
              {override && (
        ...
        <div className="mt-4 flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <button
              ...
              onClick={() => {
                apply(null);
                setDraft(toLocalInputValue(new Date()));
              }}
            >
            ...
            <div className="flex flex-wrap gap-2">
              {[1, 7, 30, 90, 180, 365].map((d) => (
                <button
                  key={d}
                  onClick={() => jump(d)}
                  className={btnSecondaryCls + " text-[12px]"}
                >
      ...
      <Card>
        ...
        <ul className="text-[12.5px] text-muted-foreground space-y-1.5 list-disc pl-5">
          <li>
            The override is stored in your browser's localStorage under{" "}
            <code className="font-mono text-[11.5px]">dev:now</code>.
          </li>
          <li>
            Every server function call sends it as an{" "}
            <code className="font-mono text-[11.5px]">x-dev-now</code> header.
          </li>
          <li>
            Server code that reads "today" (via{" "}
            <code className="font-mono text-[11.5px]">serverNow()</code>) honors it.
          </li>
          <li>
            Writes that record audit trails still use real wall-clock time — so nothing in the
            database is silently backdated.
          </li>
