...
export function EodTab() {
  ...
  const saveM = useMutation({
    ...
    onSuccess: () => { toast.success("Auto day-end settings saved"); qc.invalidateQueries({ queryKey: ["eod-settings"] }); },
    ...
    <div className="flex flex-col gap-5">
      ...
      <Card>
        ...
        <p className="text-[12.5px] text-muted-foreground mb-3">
          When enabled, the system automatically initiates the day-end process for <strong>all branches</strong> at the configured time (workspace timezone: <span className="font-mono">{settings?.timezone ?? "—"}</span>).
        </p>
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[11px] font-semibold uppercase text-faint">Run at (local time)</label>
            <input type="time" value={autoTime} onChange={(e) => setAutoTime(e.target.value)} className={cn(inputCls, "w-32")} disabled={!autoEnabled} />
          </div>
          <button className={btnPrimaryCls} onClick={() => saveM.mutate()} disabled={saveM.isPending}>
      ...
      <Card>
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <div className="text-[12px] text-muted-foreground">Runs the full day-end sequence for every branch at once.</div>
          </div>
          <div className="flex items-end gap-3">
            <div className="flex flex-col gap-1">
              <label className="text-[11px] font-semibold uppercase text-faint">Business date</label>
              <input type="date" value={businessDate} onChange={(e) => setBusinessDate(e.target.value)} className={cn(inputCls, "w-40")} />
            </div>
            <button className={btnPrimaryCls} onClick={() => runM.mutate()} disabled={runM.isPending}>
              {runM.isPending
                ...
                <td className={cn("py-2 pr-3 capitalize", STATUS_STYLE[run?.status ?? "pending"])}>
                  {run ? (
                    <span className="flex items-center gap-1.5">
                      {run.status === "completed" && <CheckCircle2 size={13} className="text-emerald-600" />}
                      {run.status === "failed" && <XCircle size={13} className="text-destructive" />}
                      {run.status === "in_progress" && <Loader2 size={13} className="animate-spin text-blue-600" />}
                      {run.status.replace(/_/g, " ")}
                    </span>
                  ) : <span className="text-muted-foreground">not started</span>}
                </td>
                <td className="py-2 pr-3">{run?.started_at ? new Date(run.started_at).toLocaleString() : "—"}</td>
                <td className="py-2 pr-3">{run?.completed_at ? new Date(run.completed_at).toLocaleString() : "—"}</td>
                <td className="py-2 pr-3 text-right font-mono">{run?.duration_ms ? `${(run.duration_ms / 1000).toFixed(1)}s` : "—"}</td>
              </tr>
            ))}
            {branchStatus.length === 0 && (
              <tr><td colSpan={6} className="py-8 text-center text-muted-foreground">No branches configured.</td></tr>
            )}
          </tbody>
        </table>
      </Card>
            ...
            {(runs ?? []).slice(0, 40).map((r) => {
              ...
              return (
                <tr key={r.id} className="border-b border-border/50">
                  ...
                  <td className="py-2 pr-3">{r.completed_at ? new Date(r.completed_at).toLocaleString() : "—"}</td>
