...
const DOMAINS: Domain[] = [
  {
    ...
    publishesEvents: [
      ...
      "loan.restructured",
      "loan.written_off",
      "loan.write_off_recovered",
      "loan.transferred_to_legal",
      "loan.charge_capitalized",
      "loan.security_attached",
      "loan.debit_note_issued",
      "loan.evaluation_submitted",
      "loan.rate_version_proposed",
      "loan.rate_version_applied",
    ],
  ...
  },
  {
    ...
    publishesEvents: [
      ...
      "fd.premature_closed",
      "fd.dormant",
      "fd.rate_version_proposed",
      "fd.rate_version_applied",
    ],
    consumesEvents: ["client.kyc_verified", "workflow.approved"],
    dependsOn: ["ledger", "clients", "workflow"],
    extractionReadiness: "ready",
  ...
  },
  {
    ...
    consumesEvents: [
      "loan.origination_requested",
      "fd.closure_requested",
  ...
  },
  {
    ...
    publishesEvents: ["api.inbound_txn_received", "api.outbound_txn_dispatched"],
    consumesEvents: [],
  ...
  },
  {
    ...
    publicApi: ["/api/public/hooks/dispatch-domain-events"],
  ...
  },
  {
    ...
    consumesEvents: ["staff.invited", "client.created", "workflow.approved", "workflow.rejected"],
    dependsOn: [],
    extractionReadiness: "ready",
    extractionNotes:
...
export function ServiceBoundaries() {
  ...
  const selected = useMemo(() => DOMAINS.find((d) => d.id === selectedId)!, [selectedId]);
    ...
    <div className="flex flex-col gap-4">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="text-[12.5px] text-muted-foreground mt-1 max-w-3xl">
              How the modular monolith maps to future services. Each domain owns its tables,
              publishes its own events and declares which other domains it depends on. Click a
              domain to inspect its contract and see how easy it would be to extract.
            </p>
          </div>
            ...
            return (
              ...
              >
                ...
                <div className="mt-2 flex items-center justify-between text-[10.5px]">
                  ...
                  <span
                    className={cn("px-1.5 py-0.5 rounded-full border font-semibold", readiness.cls)}
                  >
                    {readiness.label}
                  </span>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <Section title="Owned tables" icon={Database}>
            <div className="flex flex-wrap gap-1.5">
              {selected.ownedTables.map((t) => (
                <code key={t} className="text-[11.5px] px-2 py-1 rounded-md bg-muted font-mono">
                  {t}
                </code>
              ))}
            </div>
          ...
          <Section title="Server functions" icon={Package}>
            <div className="flex flex-wrap gap-1.5">
              {selected.serverFns.map((f) => (
                <code key={f} className="text-[11.5px] px-2 py-1 rounded-md bg-muted font-mono">
                  {f}
                </code>
              ))}
            </div>
            ...
            <Section title="Public HTTP endpoints" icon={Radio}>
              <div className="flex flex-col gap-1">
                {selected.publicApi.map((p) => (
                  <code key={p} className="text-[11.5px] px-2 py-1 rounded-md bg-muted font-mono">
                    {p}
                  </code>
                ))}
              </div>
        ...
        <Card>
          ...
          <div className="mt-4 pt-3 border-t border-border">
            ...
            <DomainChips
              ids={selected.dependsOn}
              onSelect={setSelectedId}
              empty="No sync dependencies — fully autonomous."
            />
          </div>
      ...
      <Card>
        ...
        <p className="text-[12.5px] text-muted-foreground mt-1">
          Recommended order if you decide to split. Each step is independently valuable and
          reversible.
        </p>
        <ol className="mt-3 space-y-2">
          {[
            [
              "1",
              "Ledger kernel RPC",
              "Centralize every GL write through public.post_entry(). No physical split — just a contract.",
            ],
            [
              "2",
              "Outbox tables per domain",
              "Every state change writes a domain_event row in the same transaction. Log-only initially.",
            ],
            [
              "3",
              "Extract Public API",
              "Move /api/public/v1/* to its own Worker with dedicated rate limits + observability.",
            ],
            [
              "4",
              "Extract Workflow",
              "Deploy independently; connect to core via outbox + REST. First true service split.",
            ],
            [
              "5",
              "Extract FD",
              "Highest-value isolated domain (heavy accrual jobs). Split its schema + accrual worker.",
            ],
            [
              "6",
              "Extract Savings, then Loans",
              "Once event bus is proven. Loans last because of ledger coupling.",
            ],
          ].map(([step, title, note]) => (
...
function DomainChips({
  ...
  if (ids.length === 0)
    return <div className="text-[12px] text-muted-foreground italic">{empty}</div>;
  return (
    <div className="flex flex-wrap gap-1.5">
      {ids.map((id) => {
