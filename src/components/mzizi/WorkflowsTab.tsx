  ...
  return (
    <div className="space-y-4 animate-fadein">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[12.5px] text-faint mt-0.5">
            Route any transaction type through multi-step approvals with SLAs.
          </p>
        </div>
        <button
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div
          className="grid gap-4 text-[10.5px] uppercase tracking-wider text-faint font-semibold py-3 px-5 border-b border-border bg-secondary/40"
        ...
        {workflows.length === 0 && (
          <div className="text-center text-faint text-sm py-10">No workflows yet.</div>
        )}
        {workflows.map((w: any) => {
          const joint = (w.steps ?? []).some((s: any) => (s.required_approvals ?? 1) > 1);
          const label =
            CANONICAL_TX_TYPES.find((t) => t.code === w.transaction_type)?.label ??
            w.transaction_type;
          return (
            <div
              key={w.id}
            ...
            >
              ...
              <div>
                {joint ? (
                  <span className="text-[10.5px] px-1.5 py-0.5 rounded bg-violet-500/10 text-violet-700 border border-violet-500/30">
                    Joint
                  </span>
                ) : (
                  <span className="text-[10.5px] text-faint">Sequential</span>
                )}
              </div>
              <div>
                <button
              ...
              <div className="flex gap-2 justify-end">
                ...
                <button
                  ...
                  onClick={() => {
                    if (confirm(`Delete workflow "${w.name}"?`)) del.mutate({ data: { id: w.id } });
                  }}
                >
...
function WorkflowEditor({
  ...
  const [customTx, setCustomTx] = useState(
    initial && !CANONICAL_TX_TYPES.some((t) => t.code === initial.transaction_type)
  ...
  const [steps, setSteps] = useState<StepDraft[]>(
    initial?.steps?.length
      ? initial.steps.map((s: any) => ({
          ...s,
          role: s.role ?? null,
          custom_role_id: s.custom_role_id ?? null,
          branch_id: s.branch_id ?? null,
          user_id: s.user_id ?? null,
          escalation_role: s.escalation_role ?? null,
          escalation_custom_role_id: s.escalation_custom_role_id ?? null,
        }))
      : [emptyStep(1)],
  );
  ...
  function addStep() {
    setSteps((p) => [...p, emptyStep(p.length + 1)]);
  }
    ...
    <div className="space-y-5 animate-fadein">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[12.5px] text-faint mt-0.5">
            Define ordered steps, joint approvers and SLAs.
          </p>
        </div>
        <div className="flex gap-2">
          <button className={btnSecondaryCls} onClick={onCancel}>
            Cancel
          </button>
          <button className={btnPrimaryCls} onClick={submit} disabled={save.isPending}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <label className="text-[12px] font-medium text-secondary-foreground space-y-1">
            ...
            <input
              ...
              placeholder="e.g. Loan approval > 100K"
            />
          </label>
          <label className="text-[12px] font-medium text-secondary-foreground space-y-1">
      ...
      <Card>
        <CardTitle>
          <div className="flex items-center justify-between w-full">
            <span>Steps</span>
            <button className={btnSecondaryCls} onClick={addStep}>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <label className="text-[11.5px] font-medium text-secondary-foreground space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) => updateStep(i, { approver_kind: e.target.value as any })}
                  >
                ...
                <label className="text-[11.5px] font-medium text-secondary-foreground space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateStep(i, {
                        required_approvals: Math.max(1, parseInt(e.target.value || "1", 10)),
                      })
                    }
                  />
                </label>
                ...
                {s.approver_kind !== "user" && (
                  <label className="text-[11.5px] font-medium text-secondary-foreground space-y-1">
                    ...
                    <select
                      className={selectCls}
                      value={
                        s.custom_role_id ? `c:${s.custom_role_id}` : s.role ? `s:${s.role}` : ""
                      }
                      onChange={(e) => {
                        const v = e.target.value;
                        if (v.startsWith("c:"))
                          updateStep(i, { custom_role_id: v.slice(2), role: null });
                        else if (v.startsWith("s:"))
                          updateStep(i, { role: v.slice(2), custom_role_id: null });
                ...
                {s.approver_kind === "branch_role" && (
                  <label className="text-[11.5px] font-medium text-secondary-foreground space-y-1">
                    ...
                    <select
                      className={selectCls}
                      value={s.branch_id ?? ""}
                      onChange={(e) => updateStep(i, { branch_id: e.target.value || null })}
                    >
                ...
                {s.approver_kind === "user" && (
                  <label className="text-[11.5px] font-medium text-secondary-foreground space-y-1 col-span-2">
                    ...
                    <select
                      className={selectCls}
                      value={s.user_id ?? ""}
                      onChange={(e) => updateStep(i, { user_id: e.target.value || null })}
                    >
                ...
                <label className="text-[11.5px] font-medium text-secondary-foreground space-y-1">
                  ...
                  <select
                    className={selectCls}
                    value={s.sla_action}
                    onChange={(e) => updateStep(i, { sla_action: e.target.value as any })}
                  >
                ...
                {s.sla_action === "escalate" && (
                  <label className="text-[11.5px] font-medium text-secondary-foreground space-y-1">
                    ...
                    <select
                      className={selectCls}
                      value={
                        s.escalation_custom_role_id
                          ? `c:${s.escalation_custom_role_id}`
                          : s.escalation_role
                            ? `s:${s.escalation_role}`
                            : ""
                      }
                      onChange={(e) => {
                        ...
                        if (v.startsWith("c:"))
                          updateStep(i, {
                            escalation_custom_role_id: v.slice(2),
                            escalation_role: null,
                          });
                        else if (v.startsWith("s:"))
                          updateStep(i, {
                            escalation_role: v.slice(2),
                            escalation_custom_role_id: null,
                          });
                        else
                          updateStep(i, { escalation_role: null, escalation_custom_role_id: null });
                      }}
                    >
                      <option value="">Select role…</option>
                      {activeRoles.length > 0 && (
