...
export function RiskProfilingTab() {
  ...
  const upFactor = useMutation({
    mutationFn: useServerFn(upsertRiskFactor),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["risk-scheme"] }),
    onError: (e: Error) => toast.error(e.message),
  });
  const delFactor = useMutation({
    mutationFn: useServerFn(deleteRiskFactor),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["risk-scheme"] }),
    onError: (e: Error) => toast.error(e.message),
  });
  const upOption = useMutation({
    mutationFn: useServerFn(upsertRiskOption),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["risk-scheme"] }),
    onError: (e: Error) => toast.error(e.message),
  });
  const delOption = useMutation({
    mutationFn: useServerFn(deleteRiskOption),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["risk-scheme"] }),
    onError: (e: Error) => toast.error(e.message),
  });
  const upBand = useMutation({
    mutationFn: useServerFn(upsertRiskBand),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["risk-scheme"] }),
    onError: (e: Error) => toast.error(e.message),
  });

  if (!scheme)
    return <Card className="p-6 text-sm text-muted-foreground">Loading risk scheme…</Card>;
      ...
      <Card className="p-4">
        <div className="mb-3">
          ...
          <div className="text-[11.5px] text-muted-foreground">
            Total risk % is bucketed into these bands.
          </div>
        </div>
          ...
          {BANDS.map((b) => {
            ...
            return (
              <BandEditor
                key={b}
                band={b}
                minPct={row?.min_pct ?? 0}
                maxPct={row?.max_pct ?? 0}
                onSave={(min, max) =>
                  upBand.mutate({ data: { band: b, min_pct: min, max_pct: max } })
                }
              />
            );
          })}
      ...
      <Card className="p-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <div className="text-[11.5px] text-muted-foreground">
              Categories (factors) group their options. Set the Assigned Risk Level and Risk Score
              for each option.
            </div>
          </div>
          <button
            ...
            onClick={() =>
              upFactor.mutate({
                data: {
                  code: `factor_${Date.now()}`,
                  label: "New category",
                  applies_to: "both",
                  multi_select: false,
                  sort_order: scheme.factors.length + 1,
                  active: true,
                },
              })
            }
            ...
            <tbody>
              {scheme.factors.map((f) => (
                <FactorGroup
                  ...
                  onDeleteFactor={() => {
                    if (confirm(`Delete category "${f.label}"?`))
                      delFactor.mutate({ data: { id: f.id } });
                  }}
    ...
    <div className={cn("border rounded-md p-3", tone)}>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className={
            btnSecondaryCls + " ml-auto inline-flex items-center gap-1 disabled:opacity-50"
          }
        >
          <Save size={12} /> Save
        </button>
...
function FactorGroup({
  ...
  const dirty =
    label !== factor.label ||
    code !== factor.code ||
    applies !== factor.applies_to ||
    multi !== factor.multi_select ||
    sort !== factor.sort_order ||
          ...
          {editingHeader ? (
            <div className="flex flex-wrap items-center gap-2">
              <input
                ...
                className={inputCls + " flex-1 min-w-[200px] font-semibold"}
              />
              <input
                value={code}
              ...
              <select
                value={applies}
                onChange={(e) => setApplies(e.target.value as RiskAppliesTo)}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-[10px] rounded-full bg-muted px-1.5 py-0.5 uppercase">
                {factor.applies_to}
              </span>
              {factor.multi_select && (
          ...
          <div className="inline-flex items-center gap-1">
            {editingHeader && (
              <button
                ...
                className={btnSecondaryCls + " inline-flex items-center gap-1 disabled:opacity-50"}
              >
                <Save size={12} /> Save
              </button>
            )}
            ...
            <button
              ...
              onClick={() =>
                onSaveOption({
                  ...
                  band: "low",
                  score: 0,
                  sort_order: (factor.options.length || 0) + 1,
                  active: true,
                })
              }
            >
  ...
  return (
    <tr className="border-t border-border hover:bg-row-hover">
      ...
      <td className="px-3 py-1.5">
        <select
          value={band}
          onChange={(e) => setBand(e.target.value as RiskBandLevel)}
          className={cn(selectCls, "w-full border", BAND_TONE[band])}
        >
          {BANDS.map((b) => (
