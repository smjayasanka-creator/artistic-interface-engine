...
export const Route = createFileRoute("/_authenticated/transactions/savings-deposit")({
  component: SavingsDepositPage,
});

function SavingsDepositPage() {
  ...
  const valid = accountId && amount && Number(amount) > 0 && paymentMethodValid(pay);
      ...
      >
        <Card className="p-6">
          <FormGrid>
            ...
            <PaymentMethodPicker
              allowed={["cash", "fund_transfer", "cheque"]}
              clientId={clientId}
              value={pay}
              onChange={setPay}
            />
        ...
        <FormActions>
          ...
          <button type="submit" disabled={!valid || post.isPending} className={btnPrimaryCls}>
            {post.isPending ? "Posting…" : "Post deposit"}
          </button>
        </FormActions>
      </form>
    </div>
