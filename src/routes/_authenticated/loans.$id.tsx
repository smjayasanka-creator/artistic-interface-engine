...
function LoanDetail() {
  ...
  if (isLoading || !data)
    return <div className="text-sm text-faint py-10 text-center">Loading…</div>;
  const d = data as any;
  const {
    loan,
    schedule,
  ...
  return (
    <div className="animate-fadein space-y-5">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <h1 className="text-xl font-semibold">
              Loan · <span className="font-mono">{loan.id.slice(0, 8).toUpperCase()}</span>
            </h1>
          ...
          <div className="text-[12.5px] text-muted-foreground flex items-center gap-3 flex-wrap">
            <span className="inline-flex items-center gap-1">
              <Building2 size={12} /> {loan.branch?.code ?? loan.branch?.name ?? "—"}
            </span>
            <span>· {loan.product?.name}</span>
            <span>
              · {loan.product?.interest_method} · {loan.frequency}
            </span>
          </div>
        </div>
        {loan.client && (
          ...
          >
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[11px] text-faint">
                {loan.client.phone ?? loan.client.national_id ?? "—"}
              </span>
            </div>
          </Link>
        )}
          ...
          return (
            ...
            >
              ...
              {t.key === "approvals" && approvals?.length > 0 && (
                <span className="ml-1 text-[10px] px-1.5 py-0.5 rounded-full bg-secondary text-faint">
                  {approvals.length}
                </span>
              )}
            </button>
      ...
      {tab === "application" && (
        <ApplicationTab applicationId={loan.application_id} applicationNo={loan.application_no} />
      )}
    </div>
  );
}
...
function ApplicationTab({
  ...
  if (!enabled) {
    return (
      <Card>
        <div className="text-center text-faint text-sm py-4">
          No origination record linked to this loan.
        </div>
      </Card>
    );
  }
  if (isLoading || !data)
    return (
      <Card>
        <div className="text-sm text-faint py-4 text-center">Loading application…</div>
      </Card>
    );
      ...
      <Card>
        <div className="flex items-center justify-between mb-3">
          <div className="text-sm font-semibold">
            Application <span className="font-mono">{applicationNo ?? m?.application_no}</span>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-[12.5px]">
          ...
          <Row
            k="Requested rate"
            v={m?.requested_rate_pct != null ? `${m.requested_rate_pct}%` : "—"}
          />
        ...
        {m?.purpose && (
          <div className="mt-3 text-[12.5px]">
            <span className="text-faint">Purpose:</span> {m.purpose}
          </div>
        )}
      </Card>
  ...
  return (
    <div className="space-y-5">
      ...
      <Card padded={false}>
        <div
          className={cn(
            "px-5 py-3 border-b border-border flex items-center gap-2 text-[11px] uppercase tracking-wider font-semibold",
          ...
          {arrears.count === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2 border-t border-border pt-3 flex items-center justify-between">
                ...
                <span className="font-mono text-lg font-semibold text-rose-700">
                  {money(arrears.total)}
                </span>
              </div>
              {arrears.oldest_due_date && (
                <div className="col-span-2 text-[11.5px] text-faint">
                  Oldest overdue: {shortDate(arrears.oldest_due_date)}
                </div>
              )}
            </div>
          )}
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <Card>
          <div className="text-[10.5px] uppercase tracking-wider text-faint font-semibold mb-2">
            Loan information
          </div>
  ...
  return (
    <Card padded={false}>
      ...
      >
        ...
        <div className="text-right">Paid P</div>
        <div className="text-right">Paid I</div>
        <div>State</div>
      </div>
      {schedule.length === 0 ? (
        ...
        schedule.map((s: any) => (
          <div
            key={s.seq}
            className="grid items-center text-[12.5px] py-2 px-5 border-b border-row-divider last:border-b-0"
            style={{ gridTemplateColumns: "60px 1fr 1fr 1fr 1fr 1fr 120px" }}
          >
            ...
            <div className="text-right font-mono text-muted-foreground">
              {money(Number(s.principal_paid))}
            </div>
            <div className="text-right font-mono text-muted-foreground">
              {money(Number(s.interest_paid))}
            </div>
            <div>
              <span
                className={cn(
                  "text-[10.5px] px-1.5 py-0.5 rounded border capitalize",
                  s.state === "paid"
    ...
    <Card padded={false}>
      <div className="px-5 py-3 border-b border-border flex items-center justify-between">
        ...
        <div className="text-[12px] text-faint">
          Total: <span className="font-mono font-semibold text-foreground">{money(total)}</span>
        </div>
      </div>
      {repayments.length === 0 ? (
        ...
        repayments.map((r: any) => (
          <div
            key={r.id}
            className="flex items-center justify-between px-5 py-2.5 border-b border-row-divider last:border-b-0 text-[12.5px]"
          >
            <div>
          ...
          >
            <div>
              ...
              <div className="text-[11px] text-faint capitalize">
                {c.charge?.charge_type ?? "—"} · {c.created_at ? shortDate(c.created_at) : ""}
              </div>
            </div>
    ...
    <Card padded={false}>
      ...
      {accruals.length === 0 ? (
        ...
        accruals.map((a: any, i: number) => (
          <div
            key={i}
            className="flex items-center justify-between px-5 py-2 border-b border-row-divider last:border-b-0 text-[12.5px]"
          >
...
function ApprovalsTab({ approvals }: any) {
  if (!approvals || approvals.length === 0) {
    return (
      <Card>
        <div className="text-center text-faint text-sm py-4">
          No approval workflows linked to this loan.
        </div>
      </Card>
    );
  }
          ...
          <Card key={inst.id} padded={false}>
            <div className="px-5 py-3 border-b border-border flex items-center justify-between gap-3 flex-wrap">
              <div>
                <div className="text-[13px] font-semibold">
                  {inst.workflow?.name ?? inst.transaction_type}
                </div>
                <div className="text-[11px] text-faint">
                  {inst.reference_label} · Initiated {shortDate(inst.initiated_at)}
                </div>
              </div>
            ...
            <div className="p-5 space-y-2">
              {steps.map((step: any) => {
                ...
                const isCurrent =
                  inst.status === "pending" && inst.current_step === step.step_order;
                    ...
                    {acts.length > 0 && (
                      <div className="mt-2 space-y-1 border-t border-border pt-2">
                        {acts.map((a) => (
                          <div
                            key={a.id}
                            className="flex items-start justify-between text-[11.5px]"
                          >
                            <div>
                              <span
                                className={cn(
              ...
              {inst.completed_at && (
                <div className="text-[11px] text-faint pt-1">
                  Completed {shortDate(inst.completed_at)}
                </div>
              )}
            </div>
          </Card>
...
function Metric({ label, value, tone }: { label: string; value: string; tone?: "primary" }) {
  return (
    <Card>
      ...
      <div
        className={cn("text-lg font-semibold font-mono mt-1", tone === "primary" && "text-primary")}
      >
        {value}
      </div>
    </Card>
