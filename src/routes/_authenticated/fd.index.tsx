...
export const Route = createFileRoute("/_authenticated/fd/")({
  loader: ({ context }) => {
    ...
    context.queryClient.ensureQueryData({
      queryKey: ["fd-summary"],
      queryFn: () => getFdSummary(),
    });
    context.queryClient.ensureQueryData({
      queryKey: ["fd-products"],
      queryFn: () => listFdProducts(),
    });
  },
...
function FdRegister() {
  ...
  useEffect(() => {
    setPage(1);
  }, [status, productId, from, to, pageSize]);
    ...
    <div className="animate-fadein flex flex-col gap-5">
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <Kpi
          ...
          value={`${summary?.maturing_count ?? 0}`}
          delta={money(summary?.maturing_this_month ?? 0)}
        />
      </div>
        ...
        {[
          {
            ...
            icon: PiggyBank,
            title: "New Deposit",
            desc: "Book a new fixed deposit certificate",
          ...
          },
          {
            ...
            desc: "Track deposits maturing soon",
          ...
          },
          {
            ...
            icon: LineChart,
            title: "ALCO Rates",
            desc: "Manage deposit rates in Administration",
          ...
          return c.to.startsWith("#") ? (
            <a key={c.to} href={c.to} className="group">
              {inner}
            </a>
          ) : (
      ...
      <Card>
        <div className="flex flex-wrap items-center gap-2 mb-3">
          <input
            placeholder="Search cert / client / product…"
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button
              className="inline-flex items-center gap-1.5 h-9 px-3 rounded-md text-sm border border-input hover:bg-muted"
              onClick={exportCsv}
            >
        ...
        <div
          className={cn("overflow-x-auto", isPlaceholderData && "opacity-60 transition-opacity")}
        >
