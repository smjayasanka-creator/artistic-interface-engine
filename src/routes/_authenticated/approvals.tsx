...
function Approvals() {
  ...
  const { data = [] } = useQuery({
    ...
    queryFn: () =>
      fn({ data: { mine: tab === "mine", status: tab === "all" ? "all" : "pending" } as any }),
  });
  ...
  return (
    <div className="space-y-5 animate-fadein">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[12.5px] text-faint mt-0.5">
            Multi-step approvals across every configured transaction type.
          </p>
        </div>
      ...
      <div className="grid gap-3">
        {data.length === 0 && (
          <Card>
            <div className="text-center text-faint text-sm py-8">Nothing here.</div>
          </Card>
        )}
        {data.map((inst: any) => (
          <InstanceRow key={inst.id} inst={inst} onOpen={() => setOpenInst(inst)} />
        ))}
      </div>
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {inst.overdue && (
              <span className="text-[10.5px] px-1.5 py-0.5 rounded bg-rose-500/10 text-rose-700 border border-rose-500/30">
                SLA overdue
              </span>
            )}
          </div>
          <div className="text-[11.5px] text-faint mt-0.5">
            {txLabel} · {inst.workflow?.name} · started{" "}
            {new Date(inst.initiated_at).toLocaleString()}
            {inst.amount != null && <> · {money(Number(inst.amount))}</>}
          </div>
          <div className="text-[11.5px] text-secondary-foreground mt-2">
            Step {inst.current_step} of {totalSteps}:{" "}
            ...
            {step && (
              <>
                ...
                {step.approver_kind === "user"
                  ? "specific user"
                  : step.approver_kind === "branch_role"
                    ? `role ${step.role} in branch`
                    : `role ${step.role}`}
                {step.required_approvals > 1 && (
                  <>
                    {" "}
                    · requires {inst.approvals_recorded}/{step.required_approvals} approvals
                  </>
                )}
                {step.sla_hours && (
                  <>
                    {" "}
                    · SLA {step.sla_hours}h ({step.sla_action})
                  </>
                )}
              </>
            )}
