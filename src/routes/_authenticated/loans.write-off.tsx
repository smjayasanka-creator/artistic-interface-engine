...
function WriteOffWorkspace() {
  ...
  const candidatesQ = useQuery({ queryKey: ["write-off-candidates"], queryFn: () => candidatesFn() });
  const writeOffsQ = useQuery({ queryKey: ["write-offs"], queryFn: () => writeOffsFn() });

  const [woModal, setWoModal] = useState<null | { id: string; contract_no: string | null; client_name: string; outstanding_total: number }>(null);
  ...
  return (
    <div className="animate-fadein space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Write off overdue facilities, track written-off balances and record subsequent collections.
          </p>
        </div>
      </div>
          ...
          <table className="w-full text-[12.5px]">
            ...
            <tbody>
              ...
              {candidatesQ.data?.length === 0 && (
                <tr><td colSpan={7} className="px-3 py-6 text-center text-faint">No eligible facilities.</td></tr>
              )}
              {candidatesQ.data?.map((c) => (
                <tr key={c.id} className="border-t border-row-divider">
                  ...
                  <td className="px-3 py-2 text-right font-mono font-semibold">{fmt(c.outstanding_total)}</td>
                  <td className="px-3 py-2 text-right">
                    <button
                      onClick={() => setWoModal({ id: c.id, contract_no: c.contract_no, client_name: c.client_name, outstanding_total: c.outstanding_total })}
          ...
          <table className="w-full text-[12.5px]">
            ...
            <tbody>
              {writeOffsQ.isLoading && (
                <tr><td colSpan={10} className="px-3 py-6 text-center text-faint">Loading…</td></tr>
              )}
              {writeOffsQ.data?.length === 0 && (
                <tr><td colSpan={10} className="px-3 py-6 text-center text-faint">No write-offs yet.</td></tr>
              )}
              {writeOffsQ.data?.map((w: any) => {
                ...
                return (
                  <tr key={w.id} className="border-t border-row-divider">
                    ...
                    <td className="px-3 py-2 text-right font-mono">{fmt(w.principal_written_off)}</td>
                    <td className="px-3 py-2 text-right font-mono">{fmt(w.interest_written_off)}</td>
                    <td className="px-3 py-2 text-right font-mono">{fmt(w.charges_written_off)}</td>
                    <td className="px-3 py-2 text-right font-mono font-semibold">{fmt(w.total_written_off)}</td>
                    <td className="px-3 py-2 text-right font-mono">
                      <span className={w.is_fully_recovered ? "text-emerald-600 font-semibold" : ""}>
                    ...
                    <td className="px-3 py-2 text-muted-foreground truncate max-w-[220px]" title={w.reason}>{w.reason}</td>
      ...
      {viewModal && (
        <RecoveriesViewModal writeOff={viewModal} onClose={() => setViewModal(null)} />
      )}
    </div>
  );
}

function ModalShell({ title, onCancel, children, width = "max-w-2xl" }: { title: string; onCancel: () => void; children: React.ReactNode; width?: string }) {
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={onCancel}>
      <div className={cn("bg-card rounded-xl border border-border w-full max-h-[90vh] overflow-auto", width)} onClick={(e) => e.stopPropagation()}>
...
function WriteOffModal({ loan, onCancel, onSubmit, submitting }: {
  ...
  return (
    <ModalShell title={`Write off ${loan.contract_no ?? loan.id.slice(0, 8)}`} onCancel={onCancel}>
      <div className="text-[12.5px] text-muted-foreground bg-secondary/40 border border-border rounded-md px-3 py-2">
        <div><b>{loan.client_name}</b> — outstanding <span className="font-mono">{fmt(loan.outstanding_total)}</span></div>
        <div className="mt-1">Posts the reversal to the loan product's Bad Debt Expense (or Loan Loss Provision) and records the master row. Cannot be undone.</div>
      </div>
      <FormGrid>
        <FormField label="Reason" required span={12}>
          <textarea className={cn(inputCls, "min-h-[72px]")} value={reason} onChange={(e) => setReason(e.target.value)} />
        </FormField>
        <FormField label="Charge to loan-loss provision (ECL)" span={12}>
          <label className="flex items-center gap-2 text-[13px]">
            <input type="checkbox" checked={useProvision} onChange={(e) => setUseProvision(e.target.checked)} />
            Use provision instead of expense
          </label>
        </FormField>
      </FormGrid>
      <FormActions>
        <button className={btnSecondaryCls} onClick={onCancel} disabled={submitting}>Cancel</button>
        <button
...
function RecoveryModal({ writeOff, onClose, onDone }: { writeOff: any; onClose: () => void; onDone: () => void }) {
  ...
  return (
    <ModalShell title={`Record recovery — ${writeOff.facility_no ?? writeOff.loan_id.slice(0, 8)}`} onCancel={onClose}>
      ...
      <FormActions>
        ...
        <button className={btnPrimaryCls} disabled={invalid || m.isPending} onClick={() => m.mutate()}>
...
function RecoveriesViewModal({ writeOff, onClose }: { writeOff: any; onClose: () => void }) {
  ...
  return (
    <ModalShell title={`Recoveries — ${writeOff.facility_no ?? writeOff.loan_id.slice(0, 8)}`} onCancel={onClose} width="max-w-3xl">
      <div className="text-[12.5px] bg-secondary/40 border border-border rounded-md px-3 py-2 grid grid-cols-3 gap-3">
        <div><div className="text-[10.5px] uppercase text-faint">Written off</div><div className="font-mono">{fmt(writeOff.total_written_off)}</div></div>
        <div><div className="text-[10.5px] uppercase text-faint">Recovered</div><div className="font-mono">{fmt(writeOff.total_recovered)}</div></div>
        <div><div className="text-[10.5px] uppercase text-faint">Outstanding</div><div className="font-mono font-semibold">{fmt(Number(writeOff.total_written_off) - Number(writeOff.total_recovered))}</div></div>
      </div>
      <div className="overflow-x-auto border border-border rounded-md">
        <table className="w-full text-[12.5px]">
          ...
          <tbody>
            {q.isLoading && <tr><td colSpan={7} className="px-3 py-4 text-center text-faint">Loading…</td></tr>}
            {q.data?.length === 0 && <tr><td colSpan={7} className="px-3 py-4 text-center text-faint">No recoveries yet.</td></tr>}
            {q.data?.map((r: any) => (
