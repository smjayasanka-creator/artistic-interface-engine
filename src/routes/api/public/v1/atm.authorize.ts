      ...
      POST: async ({ request }) => {
        ...
        if (parsed.data.terminal_id !== term.value) {
          return errJson({
            ...
            message: "X-Terminal-Id header must match body terminal_id.",
          });
        }
        ...
        if (hit.kind === "replay")
          return validateAndSend(AtmResponse, hit.response as any, hit.status);
        ...
        const response = {
          ...
          balance_after:
            parsed.data.transaction_type === "withdrawal"
              ? 87250
              : parsed.data.transaction_type === "balance_inquiry"
                ? 125000
                : null,
