...
import {
  CribRequest, CribResponse,
  parseJsonBody, validateAndSend, logAndReturnAuthError, errJson, requireHeader,
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        if (parsed.data.consent_reference !== consent.value) {
          return errJson({ code: 451, error: "consent_invalid", message: "X-Consent-Reference header must match body consent_reference." });
        }
        ...
        await logApiCall({ company_id: auth.key.company_id, api_key_id: auth.key.id, channel: CHANNEL, direction: "outbound",
          endpoint: ENDPOINT, method: "POST", reference, status_code: 200, request: parsed.data, response });
