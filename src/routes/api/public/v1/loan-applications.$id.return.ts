// POST /api/public/v1/loan-applications/{id}/return
...
export const Route = createFileRoute("/api/public/v1/loan-applications/$id/return")({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        const endpoint = `/api/public/v1/loan-applications/${params.id}/return`;
        ...
        const parsed = await parseJsonBody(request, LoanApplicationReturnRequest);
        if (!parsed.ok) return parsed.response;

        const { error: rpcErr } = await supabaseAdmin.rpc("return_loan_application" as any, {
        ...
        if (rpcErr) {
          ...
          return errJson({ code: 409, error: "return_failed", message: rpcErr.message });
        }
        ...
        const body = {
          ...
          returned_at: new Date(r.updated_at ?? new Date().toISOString()).toISOString(),
        };
        ...
        try {
          await enqueueWebhookForCompany(supabaseAdmin as any, {
            ...
            event_type: "loan_application.returned",
            event_id: app.id,
            payload: body,
          });
        } catch (e) {
          console.warn("[webhook] loan_application.returned enqueue failed", e);
        }
        ...
        return validateAndSend(LoanApplicationReturnResponse, body, 200);
      },
    },
  },
});
