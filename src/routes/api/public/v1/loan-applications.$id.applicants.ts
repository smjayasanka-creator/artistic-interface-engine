// POST /api/public/v1/loan-applications/{id}/applicants
...
export const Route = createFileRoute("/api/public/v1/loan-applications/$id/applicants")({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        const endpoint = `/api/public/v1/loan-applications/${params.id}/applicants`;
        ...
        const parsed = await parseJsonBody(request, LoanApplicationApplicantRequest);
        if (!parsed.ok) {
          await logApiCall({
