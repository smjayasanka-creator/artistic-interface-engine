// POST /api/public/v1/loan-applications/{id}/collateral
...
export const Route = createFileRoute("/api/public/v1/loan-applications/$id/collateral")({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        const endpoint = `/api/public/v1/loan-applications/${params.id}/collateral`;
        ...
        const parsed = await parseJsonBody(request, LoanApplicationCollateralRequest);
        if (!parsed.ok) return parsed.response;

        // Cross-tenant guard on security_type_id when provided.
        if (parsed.data.security_type_id) {
          const { data: st } = await supabaseAdmin
            .from("security_type")
            .select("id, company_id")
            .eq("id", parsed.data.security_type_id)
            .maybeSingle();
          if (!st || (st as any).company_id !== auth.key.company_id)
            return errJson({ code: 404, error: "security_type_not_found", message: "Security type not found." });
        }
        ...
        const { data: row, error } = await supabaseAdmin
          ...
          .insert({
            ...
            security_type_id: parsed.data.security_type_id ?? null,
