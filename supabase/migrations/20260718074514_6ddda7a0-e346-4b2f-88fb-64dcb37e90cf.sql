...
BEGIN
  ...
  ELSE
    ...
    -- Branch code must match ^[0-9]{3}$: pick lowest unused 3-digit code for the company.
    SELECT lpad(gs::text, 3, '0') INTO _branch_code
      FROM generate_series(1, 999) gs
     WHERE NOT EXISTS (SELECT 1 FROM public.branch b WHERE b.company_id = _company_id AND b.code = lpad(gs::text, 3, '0'))
     ORDER BY gs ASC LIMIT 1;

    INSERT INTO public.branch (company_id, code, name, currency)
      VALUES (_company_id, COALESCE(_branch_code, '001'), 'Head Office',
              COALESCE(NULLIF(NEW.raw_user_meta_data->>'currency',''), 'KES'))
