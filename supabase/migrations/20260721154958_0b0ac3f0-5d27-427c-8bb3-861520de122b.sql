...
BEGIN
  ...
  -- Branch-scoped pending Savings/FD workflow blocker. workflow_instance
  -- lacks a branch_id, so resolve branch via the referenced entity per
  -- transaction_type family.
  SELECT count(*) INTO _n FROM public.workflow_instance wi
    WHERE wi.company_id = _company_id
      AND wi.status = 'pending'
      AND (
        -- Savings-account-referenced flows (opening, closing, adjustments)
        (
          (wi.transaction_type ILIKE 'savings%' OR wi.transaction_type ILIKE '%_savings%')
          AND wi.transaction_type NOT ILIKE '%hold%'
          AND EXISTS (
            SELECT 1 FROM public.savings_account sa
            WHERE sa.id = wi.reference_id AND sa.branch_id = _branch_id
          )
        )
        -- Savings hold releases (reference_id -> savings_hold.id)
        OR (
          wi.transaction_type ILIKE '%hold%'
          AND EXISTS (
            SELECT 1
            FROM public.savings_hold sh
        ...
        OR (
          ...
          AND EXISTS (
            SELECT 1 FROM public.fixed_deposit fd
  ...
  -- Other pending workflows for this branch surface as warnings only.
  -- We only warn for items we can resolve to this branch.
  SELECT count(*) INTO _n FROM public.workflow_instance wi
    WHERE wi.company_id = _company_id
      AND wi.status = 'pending'
      AND NOT (
