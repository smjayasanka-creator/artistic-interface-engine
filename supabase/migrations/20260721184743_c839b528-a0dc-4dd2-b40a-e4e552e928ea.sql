...
BEGIN
  ...
  -- Approved loans awaiting disbursement are informational only; they never block day-end.
  SELECT count(*) INTO _n FROM public.loan l
    WHERE l.branch_id=_branch_id AND l.status='approved';
  IF _n > 0 THEN
    _warnings := _warnings || jsonb_build_array(jsonb_build_object(
      'code','inflight_disbursements','count',_n,
      'label','Approved loans awaiting disbursement (informational)'));
  END IF;
