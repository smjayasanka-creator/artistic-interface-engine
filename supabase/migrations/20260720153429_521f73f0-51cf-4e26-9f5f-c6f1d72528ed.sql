...
VALUES
  ('savings.adjustment.post', 'savings', 'Post savings adjustment', 'Post a savings account adjustment (credit or debit)', 60),
  ('savings.reversal.post',   'savings', 'Reverse savings txn',    'Reverse a previously posted savings transaction', 61)
ON CONFLICT (code) DO NOTHING;

CREATE OR REPLACE FUNCTION public.assert_savings_txn_permission(
  _company_id uuid,
  _txn_type   text
) RETURNS void
LANGUAGE plpgsql
...
DECLARE
  v_required text;
BEGIN
  IF public.is_company_admin(_company_id) THEN
    RETURN;
  END IF;
  v_required := CASE _txn_type
    WHEN 'deposit'      THEN 'savings.deposit.post'
    WHEN 'transfer_in'  THEN 'savings.transfer.post'
    WHEN 'withdrawal'   THEN 'savings.withdraw.post'
    WHEN 'transfer_out' THEN 'savings.transfer.post'
    WHEN 'adjustment'   THEN 'savings.adjustment.post'
    WHEN 'fee'          THEN 'savings.adjustment.post'
    WHEN 'interest'     THEN 'savings.interest.process'
    ELSE NULL
  END;
  IF v_required IS NULL THEN
    RAISE EXCEPTION 'Unsupported savings txn_type %', _txn_type
      USING ERRCODE = '42501';
  END IF;
  IF NOT public.has_permission(auth.uid(), v_required, _company_id) THEN
    RAISE EXCEPTION 'Missing permission: %', v_required
      USING ERRCODE = '42501';
  END IF;
END $$;

GRANT EXECUTE ON FUNCTION public.assert_savings_txn_permission(uuid, text) TO authenticated;
...
CREATE OR REPLACE FUNCTION public.reverse_savings_txn(_txn_id uuid, _reason text)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF NOT public.is_company_admin(v_orig.company_id)
     AND NOT public.has_permission(auth.uid(), 'savings.reversal.post', v_orig.company_id) THEN
    RAISE EXCEPTION 'Missing permission: savings.reversal.post' USING ERRCODE = '42501';
  END IF;
...
BEGIN
  ...
  IF NOT public.is_company_admin(v_from.company_id)
     AND NOT public.has_permission(auth.uid(), 'savings.transfer.post', v_from.company_id) THEN
    RAISE EXCEPTION 'Missing permission: savings.transfer.post' USING ERRCODE = '42501';
  END IF;
...
DROP POLICY IF EXISTS "company members create savings txns" ON public.savings_transaction;
REVOKE INSERT, UPDATE, DELETE ON public.savings_transaction FROM authenticated;

DROP POLICY IF EXISTS "company members create savings accounts" ON public.savings_account;
DROP POLICY IF EXISTS "company members update savings accounts" ON public.savings_account;

CREATE POLICY "savings accounts insert (open perm or admin)"
  ON public.savings_account
  FOR INSERT
  TO authenticated
  WITH CHECK (
    ...
    AND (
      ...
      OR public.has_permission(auth.uid(), 'savings.accounts.open', company_id)
      OR public.has_permission(auth.uid(), 'savings.open', company_id)
    )
  );

CREATE POLICY "savings accounts update (open/close perm or admin)"
  ON public.savings_account
  FOR UPDATE
  TO authenticated
  ...
  WITH CHECK (
    ...
    AND (
      ...
      OR public.has_permission(auth.uid(), 'savings.accounts.open', company_id)
      OR public.has_permission(auth.uid(), 'savings.open',          company_id)
      OR public.has_permission(auth.uid(), 'savings.close',         company_id)
      OR public.has_permission(auth.uid(), 'savings.admin',         company_id)
    )
  );

DROP POLICY IF EXISTS sah_write ON public.savings_account_holder;
CREATE POLICY sah_write ON public.savings_account_holder
  FOR ALL
  TO authenticated
  USING (public.is_company_member(company_id))
  WITH CHECK (
...
DROP POLICY IF EXISTS san_write ON public.savings_account_nominee;
CREATE POLICY san_write ON public.savings_account_nominee
  FOR ALL
  TO authenticated
  USING (public.is_company_member(company_id))
  WITH CHECK (
...
DROP POLICY IF EXISTS sam_write ON public.savings_account_mandate;
CREATE POLICY sam_write ON public.savings_account_mandate
  FOR ALL
  TO authenticated
  USING (public.is_company_member(company_id))
  WITH CHECK (
