...
BEGIN
  ...
  WITH movements AS (
    ...
    LEFT JOIN public.savings_transaction st ON st.account_id=sa.id AND st.txn_date=_business_date
  ...
  RETURN jsonb_build_object('savings',_savings_count,'fd',_fd_count,'loan',_loan_count,'gl',_gl_count);
END;
$function$;